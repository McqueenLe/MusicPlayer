	...
	private final BroadcastReceiver sdCardReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals("android.intent.action.MEDIA_REMOVED")// 各种未挂载状态
				...
				finish();
				Toast.makeText(MainContentActivity.this, "SD卡已拔出或失效",
...
//		getWindowManager().getDefaultDisplay().getMetrics(metric);
		...
		final Dialog dialog = new Dialog(MainContentActivity.this, R.style.round_corner_dialog);
	...
	private void setSleepClock(String timeS) {
		...
		PendingIntent pendingIntent = PendingIntent.getBroadcast(
				MainContentActivity.this, 0, intent, 0); // 定时任务完成后会发送一个广播，广播接收者会结束掉应用
		// 设置time时间之后退出程序
